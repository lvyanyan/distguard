...
export function renderTerminal(
  ...
  for (const [file, findings] of byFile) {
    ...
    for (const finding of findings) {
      ...
      const loc =
        finding.length > 0 ? paint(`${finding.line}:${finding.column}`.padEnd(7), finding.severity) : ''.padEnd(7)
      const row =
        `  ${loc}  ${paint(finding.severity.padEnd(8), finding.severity)}  ${paint(finding.ruleId, finding.severity)}  ${finding.preview}`
      console.log(row)
  ...
  console.log(paint(`✖ ${report.findings.length} problems`, 'critical') +
